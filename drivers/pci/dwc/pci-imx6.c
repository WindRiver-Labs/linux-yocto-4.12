...
static struct dw_pcie_host_ops imx_pcie_host_ops = {
...
static int imx_pcie_probe(struct platform_device *pdev)
{
	...
	switch (imx_pcie->variant) {
	case IMX7D:
		imx_pcie->pciephy_reset = devm_reset_control_get(dev,
								  "pciephy");
		if (IS_ERR(imx_pcie->pciephy_reset)) {
			dev_err(dev, "Failed to get PCIEPHY reset control\n");
			return PTR_ERR(imx_pcie->pciephy_reset);
		}

		imx_pcie->apps_reset = devm_reset_control_get(dev, "apps");
		if (IS_ERR(imx_pcie->apps_reset)) {
			dev_err(dev, "Failed to get PCIE APPS reset control\n");
			return PTR_ERR(imx_pcie->apps_reset);
		}
		break;
	default:
		break;
	...
	if (imx_pcie->variant == IMX6QP) {
		imx_pcie->pcie_bus_regulator = devm_regulator_get(pci->dev,
				"pcie-bus");
	...
	if (imx_pcie->variant == IMX7D) {
		...
		imx_pcie->pcie_phy_regulator = devm_regulator_get(pci->dev,
				"pcie-phy");
	} else if (imx_pcie->variant == IMX8MQ) {
		imx_pcie->iomuxc_gpr =
			 syscon_regmap_lookup_by_compatible
