...
static int mxsfb_set_pixel_fmt(struct mxsfb_drm_private *mxsfb, bool update)
{
	...
	format_name = drm_get_format_name(format, &format_name_buf);
...
{
	struct drm_crtc *crtc = &mxsfb->pipe.crtc;
	unsigned int bits_per_pixel = crtc->primary->state->fb->format->cpp[0]*8;
	struct drm_device *drm = crtc->dev;
			       ...
			       struct drm_plane_state *old_state)
{
	...
	if (old_fb->format->format != fb->format->format) {
		const char *old_fmt_name = drm_get_format_name(old_fb->format->format, &format_name_buf);
		const char *new_fmt_name = drm_get_format_name(fb->format->format, &format_name_buf);
