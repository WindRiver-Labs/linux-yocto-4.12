...
static int axi_hdmi_load(struct drm_device *dev, unsigned long flags)
{
	...
	private->fbdev = drm_fbdev_cma_init(dev, 32, 1);
...
static int axi_hdmi_get_platform_dev(struct platform_device *platdev,
...
static int axi_hdmi_platform_probe(struct platform_device *pdev)
{
	...
	return axi_hdmi_get_platform_dev(pdev, &axi_hdmi_driver);
}
...
static void axi_hdmi_platform_shutdown(struct platform_device *pdev)
{
	struct axi_hdmi_private *private = platform_get_drvdata(pdev);
...
static struct platform_driver adv7511_encoder_driver = {
	...
	.shutdown = axi_hdmi_platform_shutdown,
};
