	...
	struct dpu_client_platformdata *pdata, struct drm_device *drm)
{
	...
	struct drm_crtc *drm_crtc = &dpu_crtc->base;
	int i, ret;
	...
	drm_crtc->port = pdata->of_node;
	drm_crtc_helper_add(drm_crtc, &dpu_helper_funcs);
	drm_crtc_init_with_planes(drm, drm_crtc, &dpu_crtc->plane[0]->base, NULL, &dpu_crtc_funcs, NULL);
